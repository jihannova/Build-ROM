...
sync () {
    cd ~/rom
    time rclone copy znxt:ccache/$ROM_PROJECT/maple/.repo.tar.zst ~/rom -P
...
retry_cacche () {
	...
	if [ $hit_rate -lt 99 ]; then
	    ...
	    git clone ${TOKEN}/jihannova/Build-ROM -b nad-13_maple ${DEVICE} && cd ${DEVICE}
	    git commit --allow-empty -m "Build maple: $(date -u +"%D %T%p %Z")"
	    git push origin HEAD:nad-13_maple
	fi
}
